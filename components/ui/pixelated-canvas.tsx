...
type PixelatedCanvasProps = {
  src: string;
  /** Fixed width in pixels. If undefined, canvas will take full container width. */
  width?: number;
  /** Fixed height in pixels. If undefined, canvas will take full container height. */
  height?: number;
  cellSize?: number;
  dotScale?: number;
  shape?: "circle" | "square";
...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  const computeSamples = React.useCallback(
    (
      ...
      for (let y = 0; y < offscreen.height; y += cellSize) {
        ...
        for (let x = 0; x < offscreen.width; x += cellSize) {
          ...
          // Dropout based on local contrast
  ...
  const drawFrame = React.useCallback(() => {
    ...
    if (!canvas || !dims || !samples.length) return;
  ...
  React.useEffect(() => {
    ...
    const updateSizeAndSamples = () => {
      if (cancelled) return;

      const displayWidth = fixedWidth ?? container?.clientWidth ?? 400;
      const displayHeight = fixedHeight ?? container?.clientHeight ?? 500;
      ...
      if (img.complete && img.naturalWidth) {
        computeSamples(img, displayWidth, displayHeight);
      }
    };
    ...
    // Resize handling when using container size (w-full/h-full)
    let resizeObserver: ResizeObserver | null = null;
    if (!fixedWidth || !fixedHeight) {
      resizeObserver = new ResizeObserver(updateSizeAndSamples);
  ...
  return (
    <div ref={containerRef} className={className ?? "w-full h-full"}>
      <canvas
        ...
        aria-label="Pixelated interactive rendering of source image"
        role="img"
      />
    </div>
  );
